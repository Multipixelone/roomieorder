...
def test_ping_empty_url_is_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_get(*a: object, **k: object) -> _OkResp:
        calls.append("hit")
        return _OkResp()

    monkeypatch.setattr("roomieorder.heartbeat.httpx.get", fake_get)
...
def test_ping_swallows_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roomieorder.heartbeat.httpx.get", boom)
