...
A button on the Home Assistant dashboard → an item gets ordered from Costco automatically → the purchase and price land in a Google Sheet. No confirm step; roommates are trusted.

---

## 1. The honest framing (read once, then never again)

Costco has no consumer purchase API, so "order it" means a real browser driving real checkout. Talon uses **Playwright in headed mode against a persistent, already-logged-in Chromium profile**. This works well but is brittle and against Costco's ToS:

- Selectors drift when Costco redesigns checkout.
- Costco fronts the site with **Akamai bot detection**, far more aggressive than most retailers'. It can serve a hard "Access Denied" / "Pardon Our Interruption" wall to an automated profile, even one that's logged in — this may block the approach outright.
- Costco can still throw an email-OTP or CAPTCHA challenge even with 2FA off, especially from a new automation pattern.
...
**Why the queue:** intake is instant and always-on; execution needs the graphical session. If the desktop is asleep, requests sit in the queue and drain on wake. It also gives free retries on Costco flakiness. Intake never blocks on the browser.
...
The "kind I like" lives here. You populate the item numbers once.
...
- **You log into Costco once, by hand, into that profile.** With no 2FA the session persists for a long time.
- Buy flow per item (Costco has no one-click Buy Now):
  1. Goto the product page (`item.url` with its slug; fall back to the slugless `.product.<item_number>.html`).
  2. Read the live price; if `> price_ceiling`, abort + alert.
  3. Click **Add to Cart** → go to the cart → **Checkout**.
  4. Leave the default (delivery) shipping address + saved payment; continue to the review page.
  5. **If `DRY_RUN`: screenshot the review page, log "would order," stop here.**
  6. Else click **Place Order**.
...
Columns: `timestamp | item_key | title | item_number | qty | unit_price | order_total | order_id | status | requester | notes`
...
- Costco login is *not* stored as a credential — it lives in the persistent browser profile after your one-time manual login.
...
- **Costco/Akamai challenge or hard bot wall** even without 2FA; handled by the pause-and-alert path, but it means it's never 100% unattended — and Akamai may block the automated profile entirely.
...
3. ⏳ **Playwright on NixOS** — browsers pinned + module wired; the one-time
   manual Costco login into the persistent profile happens on deploy.
...
7. ⏳ **Fill the catalog** — replace the placeholder item numbers + URLs with real ones + add each dashboard button.
